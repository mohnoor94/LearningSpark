...
/** Finds the degrees of separation between two Marvel comic book characters, based
  ...
  */
object _08_DegreesOfSeparation {
  ...
  def main(args: Array[String]) {
    ...
    if (startCharacterID == targetCharacterID)
      println("How far am I from myself? I am myself! ya ya, it's a ZERO!")
    else
      for (iteration <- 1 to 10) {
        ...
        if (hitCounter.isDefined) {
          ...
          if (hitCount > 0) {
            ...
            println(s"\n>>> Hero '$startCharacterID' is $iteration step(s) away from hero '$targetCharacterID'")
            return
          }
        }
  ...
  def convertToBFS(line: String): BFSNode = {
    ...
    val connections = fields.drop(1).map(_.toInt)

    // Default distance and color is Int.MaxValue and white
    // Unless this is the character we're starting from
    val isThisMe = heroID == startCharacterID
    val color = if (isThisMe) "GRAY" else "WHITE"
    val distance = if (isThisMe) 0 else Int.MaxValue
